...
const buildAlbumDataModule = (dataRoot: string, watchFile: (file: string) => void) => {
  const albums = readdirSync(dataRoot, { withFileTypes: true })
    ...
    .map(entry => {
      ...
      const manifest = v.parse(albumManifestSchema, JSON.parse(readFileSync(manifestPath, 'utf8')))
      return {
        id: manifest.id,
        folderName: entry.name,
        coverFilename,
        rows: JSON.parse(readFileSync(metadataPath, 'utf8')) as unknown,
      }
    })
